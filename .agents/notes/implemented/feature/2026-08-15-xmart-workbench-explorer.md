...
A session with a cwd can open Explorer from `+`, browse, create files/folders, @ a path into the composer, and open several files as editor tabs. Saving writes through `ctx.workspaces`. Agent file tools refresh the tree and banner open editors. Image and binary paths open placeholder tabs. Git, terminal, tasks, and split panes still wait. `conversation.view` and `agent-loop` stay untouched.
