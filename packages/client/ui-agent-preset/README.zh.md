...
agent preset 的各个表层：General 设置中的一行，用于选择所有已列出会话以及此后新建会话据以组装的 [preset](../../preset/agent-presets/README.md)；新建会话界面上的一枚 chip，用于选择**下一个会话**的 preset；会话标题旁的选择器，用于选择**本会话**的 preset；以及一个设置页分区，用于管理名单——复制、删除、默认值，以及通往 preset 自身文件的入口。

## 两套作用域

设置里的写入（General 行或「设为默认」）是全局的：它记下部署默认值，并把每一个已列出的根会话重组到该 preset。会话标题旁的选择只是局部的：它只对当前会话调用 `agentPreset.select`，不写默认值。子代理行不会被全局写入改写；它们在派生时继承。
...
已经开始的会话不是暂存选择的目标：暂存值会被丢弃，而不是去改 chip 并没有对着看的那场对话。已开始的会话从标题旁的选择器切换。

## 会话标题旁的选择器

第三个表层，位于会话标题旁：**本会话**所运行的 preset，作为控件呈现。另选一个 preset 只重组这一场会话。它从会话自身的摘要读取 preset，并在 General 行所读的同一份名单上解析显示名称。转发的 owner 事件 `agent-preset/selected` 会在每个标签页中把已经提交的切换折进这份共享摘要；发起方标签页可能已经采用 RPC 回执，而合并是幂等的。
...
名单行携带 `broken`（宿主的形状检查发现组装缺失或不可加载）时渲染为标记卡片：红色边框、「加载失败」徽记（discovery 观察到的事实，而非断言文件已损坏——常见起因是用户刚编辑或删除了组装文件）、原样展示的原因、卡片主体禁用——它不能成为默认——复制也禁用，因为损坏 preset 的副本只是又一个损坏的 preset。损坏的自定义行保留位置与删除动作：文件正是修复它的地方，而删除正是清掉幽灵目录（组装文件被手动删除、目录仍占着 id）的方式；损坏的内置行连查看器也不提供——没有可读的组装可展示。三个选择器（通用设置行、新会话 chip 与会话标题旁的控件）则完全不列出损坏的 preset：它们选的是必须能跑起来的组装，列出无法组装的选项只会把失败推迟。
...
未组装任何 preset 的部署返回空名单，本行、chip、标题旁选择器与分区都不渲染任何内容——此时每个会话共用宿主组装，也就无从选择或管理。未配置可写根目录的部署返回 `authorable: false`，分区随之退化为只读浏览：随附组装仍可在查看器中打开，但每个复制操作都被禁用并以原因作提示，而不是给出一个创建必然失败的对话框。
...
更改默认值会重组已列出的会话，因此这些会话此后的轮次会从新组装建立新的前缀。标题旁的选择对单场会话做同样的事。历史上的工具调用仍对照产生它们的那份 preset 的常驻挂载解析。
