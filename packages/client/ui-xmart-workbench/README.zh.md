...
Git 仓库选择器只探测当前会话文件夹（和资源管理器同一棵根）以及登记在它下面的 Workspace 路径。右侧栏其它项目不会进下拉。换到另一个项目的会话会换种子。父目录本身不是仓库时，仍贡献它的一层子仓。同一仓库内的多个 Workspace 路径会按 `host.gitStatus` 返回的规范仓库根去重。
