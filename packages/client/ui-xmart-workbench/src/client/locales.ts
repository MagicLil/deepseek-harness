...
export const zh: Record<WorkbenchKey, string> = {
  ...
  'problems.empty': '这里汇总两类结果：①已打开编辑器里的语言诊断；②底栏「检查」跑完后解析出的错误。未打开文件里的类型/语法问题不会自动出现——请先在「检查」里「全部运行」，或打开相关源文件。',
  ...
  'checks.blurb': '在检查根目录执行 package.json 的 typecheck / lint / test / build（与 CI 同类），不是编辑器里的实时诊断。「运行相关」只带上 Git 改动的代码文件；「全部运行」跑完整脚本。',
...
export const en: Record<WorkbenchKey, string> = {
  ...
  'problems.empty': 'Two sources only: (1) language diagnostics from open editors; (2) errors parsed after Checks finishes. Issues in closed files will not appear automatically — Run All in Checks, or open the related source files.',
  ...
  'checks.blurb': 'Runs package.json typecheck / lint / test / build at the package root (CI-style), not live editor diagnostics. Related = dirty code files only; Run All = full scripts.',
