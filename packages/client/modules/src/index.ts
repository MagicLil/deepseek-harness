...
export class ClientModuleRegistry extends Service {
   ...
   * HTTP `/plugins` routes and index taps register only when `webServer` is
   * present (browser surface). Desktop composes the same graph and serves
   * bundles through the Electron IPC/`dsh://` shell instead.
  ...
  constructor(ctx: Context) {
    ...
    // Optional HTTP carriage — absent on the desktop Electron composition.
