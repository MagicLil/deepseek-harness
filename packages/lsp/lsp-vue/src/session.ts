/**
 * One persistent Vue language-server process for one workspace. Editor
 * documents stay open; agent queries reuse an open buffer or transient-open.
 * @module @deepseek-ai/dsh-lsp-vue/session
 */
...
import { VueLspConnection } from './connection.ts'
import type { ConnectionSpawner, VueConnectionSpec } from './connection.ts'
import { fileUrlFor, normalizeCompletions, normalizeDiagnostics } from './protocol.ts'
import type { VueLspCompletionItem, VueLspDiagnostic } from './types.ts'

/** Launch + initialize parameters for one workspace session. */
export interface VueSessionSpec extends VueConnectionSpec {
  /** Canonical workspace directory (subprocess cwd and path joins). */
  readonly workspacePath: string
  /** Canonical workspace file URI. */
  readonly workspaceUri: string
  /** Static `initialize` options (must include `typescript.tsdk`). */
...
/** One initialized Vue language-server process. */
export class VueLspSession {
  private readonly connection: VueLspConnection
  private readonly docs = new Map<string, OpenDoc>()
  private readonly diagnostics = new Map<string, VueLspDiagnostic[]>()
  ...
  constructor(private readonly spec: VueSessionSpec, spawner: ConnectionSpawner) {
    this.connection = new VueLspConnection(
      spec,
      spawner,
  ...
  diagnosticsFor(workspaceRoot: string, filePath: string): VueLspDiagnostic[] {
  ...
  navigate(
    ...
    return this.enqueue(signal, async () => {
      ...
      const hoverDeadline = deadline(
        signal,
        operation === 'hover' ? HOVER_TIMEOUT_MS : 0,
        'LSP_HOVER',
      )
      try {
