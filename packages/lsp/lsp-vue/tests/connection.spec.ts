...
import { VueLspConnection, defaultServerRequest } from '../src/connection.ts'
import { fixtureServer } from './helpers.ts'

let root: string
let live: VueLspConnection[] = []

beforeEach(async () => {
  root = await realpath(await mkdtemp(join(tmpdir(), 'lsp-vue-conn-')))
...
function connect(env: Record<string, string> = {}): VueLspConnection {
  const notes: Array<{ method: string; params: unknown }> = []
  const connection = new VueLspConnection({
...
describe('VueLspConnection', () => {
  it('initializes, notifies, and captures publishDiagnostics', async () => {
    ...
    await connection.notify('textDocument/didOpen', {
      textDocument: { uri: 'file:///ws/a.vue', languageId: 'vue', version: 1, text: 'x' },
    })
  ...
  it('fails the instance on a framing error', async () => {
    const connection = connect()
    await connection.request('initialize', { processId: null, capabilities: {} })
    // A second initialize is still framed; send garbage by writing a huge Content-Length via notify
    // is still valid JSON. Use a child that already answered — instead terminate and wait.
    connection.terminate()
