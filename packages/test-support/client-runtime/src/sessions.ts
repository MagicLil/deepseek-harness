...
export class FixtureSession implements SessionFace {
  ...
  /**
   * Fail-loud stub; supply `open` on the fixture's session face to exercise it.
   * @returns never — always throws.
   */
  open(): never {
    throw new Error(`test session "${this.sessionId}": open is not stubbed — supply it on the fixture's session face`)
  }
