...
describe('fixture session face', () => {
  it('fail-loud stubs name the missing verb; supplied overrides run instead', async () => {
    ...
    expect(() => bare.open()).toThrow(/open is not stubbed/)
...
describe('workspaces action face', () => {
  it('records every IWorkspaces verb with inert defaults and honors stubs', async () => {
    ...
    await expect(ws.renameEntry('/proj/file.ts', 'renamed.ts')).resolves.toBe('/proj/renamed.ts')
    await expect(ws.renameEntry('D:\\ws\\a.ts', 'b.ts')).resolves.toBe('D:\\ws\\b.ts')
    await ws.deleteEntry('/proj/renamed.ts')
    await expect(ws.readFileBytes('/proj/a.png')).resolves.toEqual({
      bytes: new Uint8Array(), mimeType: 'application/octet-stream',
    })
    ...
    expect(ws.calls.map(c => c.method)).toEqual(
      ['create', 'create', 'pickDirectory', 'rename', 'delete', 'openPath', 'renameEntry', 'renameEntry', 'deleteEntry', 'readFileBytes', 'insertBefore', 'insertSessionBefore', 'archiveSession', 'refresh'])
    ...
    ws.stub('renameEntry', () => Promise.resolve('/s/z.ts'))
    ws.stub('deleteEntry', () => Promise.resolve())
    ws.stub('readFileBytes', () => Promise.resolve({ bytes: new Uint8Array([1]), mimeType: 'image/png' }))
    ...
    await expect(ws.renameEntry('/s/a.ts', 'z.ts')).resolves.toBe('/s/z.ts')
    await ws.deleteEntry('/s/z.ts')
    await expect(ws.readFileBytes('/s/a.png')).resolves.toEqual({
