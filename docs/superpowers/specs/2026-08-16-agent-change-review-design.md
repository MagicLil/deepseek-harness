...
| UI 落点 | **A（Cursor）**：对话输入框上方 `conversation.input.dock` 审查条；**无**活动栏「审查」入口，也无工具卡片上的审查药丸 |
| 跟踪范围 | **C+B**：以 `write` / `edit`（及同族）为主；壳命令启发式解析删除路径进审查；解析不到则弱提示。不透明子代理（`cursor_agent` / `subagent*`）用调用前后的 `git status` 差，外加提示词/结果里的路径，导入审查行 |
