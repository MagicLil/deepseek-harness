...
- `commit-msg` requires a conventional type (`feat`, `fix`, `docs`, `style`, `refactor`, `perf`, `test`, `build`, `ci`, `chore`, `release`, or `revert`) and Chinese for the rest of the first line. `Merge`, `Revert`, `fixup!`, and `squash!` subjects are skipped.
