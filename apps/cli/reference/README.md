...
`dsh desktop` is a hardcoded alias for `--profile desktop`. The Node CLI re-execs under Electron so Cordis owns the main process; the desktop bundle opens a `dsh://` window and carries `/api` over IPC (no `webserver`).
